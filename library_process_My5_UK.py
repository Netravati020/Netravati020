...
class process(BaseCrawling):
    ...
    def Tv_show_url_extracting(self):
        ...
        try:
            ...
            tv_show_urls_df = self.create_input_xlsx_file(tv_show_urls,self.utils.library_filename + self.tv_show_url_input_file_name+".xlsx")
    ...
    def Tv_show_data_extracting(self):
        ...
        try:
            ...
            # Read columns from config
            # series_input_url_df=series_input_url_df.head(3)
    ...
    def parse_episode_data(self, epi_detail):
        ...
        episode_synopsis_xpath= self.utils.xpaths_dict['tv_show_episode_synopsis']
        ep_links = []
        # get episode links
        ...
        if "Season" in epi_detail.text:
            ...
            Episode_name = self.crawl_wrapper.inner_element(epi_detail, episode_name_xpath)

            Episode_Synopsis=self.crawl_wrapper.inner_element(epi_detail,episode_synopsis_xpath)

            try:
                Episode_no=self.crawl_wrapper.inner_element(epi_detail,episode_no_xpath)
            except:
                Episode_no=""
            show_url = self.crawl_wrapper.driver.current_url

            title = self.crawl_wrapper.driver.title.split('-')[1].strip()

            rating = self.crawl_wrapper.find_info("xpath", tv_show_rating_xpath, type_of_element='element')

            syn = self.crawl_wrapper.find_info("xpath", tv_show_synopsis_xpath, type_of_element='element')

            season_no = self.crawl_wrapper.find_info("xpath", tv_show_season_no_xpath,type_of_element='element').replace('Season', '').strip()

            dur = self.crawl_wrapper.find_info("xpath", tv_show_duration_xpath, type_of_element='element')
            duration = dur.split(' | ')[0].replace(' min', '')
            genre = dur.split(' | ')[1]
    ...
    def episodes(self):
        ...
        for epi_detail in episode_details:
            self.parse_episode_data(epi_detail)